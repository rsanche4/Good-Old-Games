...
public class Game extends JFrame implements Runnable {
	...
	public static int MAX_WORLD_LIMIT = 26;
	...
	public static void main(String[] args) {
		try {
			...
			SCREEN_W = 640;//screenSize.width;
			SCREEN_H = 640;//screenSize.height;
				...
				if (world_name.contentEquals(current_world)) {
					...
					for (int row = 0; row < MAX_WORLD_LIMIT; row++) {
						for (int col = 0; col < MAX_WORLD_LIMIT; col++) {
							layer0[row][col] = "block_floor.png";//layer0_flat[row * MAX_WORLD_LIMIT + col];
						}
					}
